import { SUNNY_SYSTEM_PROMPT, CROWLEY_SYSTEM_PROMPT, DEBATE_JUDGE_PROMPT } from '../constants/characters.js'
...
async function chatCompletion(apiKey, systemPrompt, messages, temperature = 0.9) {
 ...
 */
function buildContextMessages(dilemma, conversationHistory, opponentLastMessage, characterId) {
  const messages = []

  // Add history context
  if (conversationHistory.length > 0) {
    const historyText = conversationHistory
      .slice(-6) // last 3 rounds
      .map(round => {
...
export async function getSunnyResponse(apiKey, dilemma, conversationHistory, crowleyMessage) {
  const messages = buildContextMessages(dilemma, conversationHistory, crowleyMessage, 'sunny')
  return chatCompletion(apiKey, SUNNY_SYSTEM_PROMPT, messages)
}

export async function getCrowleyResponse(apiKey, dilemma, conversationHistory, sunnyMessage) {
  const messages = buildContextMessages(dilemma, conversationHistory, sunnyMessage, 'crowley')
  return chatCompletion(apiKey, CROWLEY_SYSTEM_PROMPT, messages)
}

export async function getDebateJudgement(apiKey, dilemma, sunnyResponse, crowleyResponse, conversationHistory, currentAlignment) {
  const prompt = `
DILEMMA: "${dilemma}"
